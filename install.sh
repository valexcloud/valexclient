...
ask_license() {
  echo "* License Key is required! If you don't have one, register for one here:"
  echo "* https://www.valexcloud.com/valexclient"
  echo -e -n "* Please input your Valex Client License Key: "
  read -r LICENSE
}
...
main() {
  ...
  readarray -t valid_timezones <<<"$(curl -s https://raw.githubusercontent.com/valexcloud/valexclient/main/configs/valid_timezones.txt)"
  ...
  # set FQDN
  while [ -z "$FQDN" ]; do
    echo -n "* Set the FQDN of Valex Client (client.example.com): "
    read -r FQDN
  ...
  [ "$CONFIGURE_LETSENCRYPT" == true ] || [ "$ASSUME_SSL" == true ] && bash <(curl -s https://raw.githubusercontent.com/valexcloud/valexclient/main/lib/verify-fqdn.sh) "$FQDN"
